...
const handleRemoveUp = (ingredienteToRemove) => {
    // Filtrar IngredienteName para eliminar el ingredienteToRemove
    const newIngredienteName = IngredienteNameUp.filter(ingrediente => ingrediente.idIngrediente !== ingredienteToRemove.idIngrediente);
    setIngredienteName(newIngredienteName);

    // Eliminar la cantidad del ingredienteToRemove de cantidades
    const newCantidades = { ...cantidades };
    delete newCantidades[ingredienteToRemove.idIngrediente];
...
const actualizarPlatillo = async () => {
    ...
    // Aquí, asegúrate de que solo estás enviando los ingredientes que están en IngredienteName
    platilloActualizado.ingredientes = platilloActualizado.ingredientes.filter(ingrediente => 
        IngredienteNameUp.some(i => i.idIngrediente === ingrediente.idIngrediente)
    );
    console.log(platilloActualizado);
    const token = localStorage.getItem('token');
    ...
    for (let ingrediente of platilloActualizado.ingredientes) {
        ...
        const ingredientesResponse = await fetch(`${API_BASE_URL}/platilloingredientes/${selectedPlatillo.idPlatillo}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
const handleUpdateChange = (event) => {
    ...
    const newCantidades = { ...cantidades };
    for (let ingrediente of IngredienteNameUp) {
